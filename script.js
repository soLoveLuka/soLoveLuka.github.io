    ...
    errorHandler.attemptRecovery(() => {
        ...
        function initializePage() {
            ...
            errorHandler.attemptRecovery(() => {
                ...
                /* ===========================================================
                   MASTERING LAB A/B BOOTSTRAP (CORS + PRELOAD FIX)
                   -----------------------------------------------------------
                   - Ensures audio created for .ab-compare blocks will use
                     crossOrigin=anonymous and preload=auto.
                   - Defensive: will NOT duplicate UI if you already rendered
                     audio elements elsewhere; it upgrades them in place.
                   =========================================================== */
                (function bootstrapMasteringLab() {
                    const abBlocks = document.querySelectorAll('.ab-compare');
                    if (!abBlocks.length) {
                        console.info('Mastering Lab: no .ab-compare blocks found.');
                        return;
                    }

                    abBlocks.forEach((block) => {
                        errorHandler.attemptRecovery(() => {
                            const aSrc = block.dataset.aSrc;
                            const bSrc = block.dataset.bSrc;
                            const title = block.dataset.title || 'A/B Comparison';

                            if (!aSrc || !bSrc) {
                                console.warn('Mastering Lab: Missing data-a-src or data-b-src on block:', block);
                                return;
                            }

                            // Find or create wrapper once
                            let wrapper = block.querySelector('.ab-wrapper');
                            if (!wrapper) {
                                wrapper = document.createElement('div');
                                wrapper.className = 'ab-wrapper';
                                // Minimal markup: two columns + label row if not present
                                wrapper.innerHTML = `
                                  <div class="ab-head">
                                    <h4 class="ab-title">${title}</h4>
                                  </div>
                                  <div class="ab-players">
                                    <div class="ab-side ab-side-a">
                                      <span class="ab-label">A • Before</span>
                                      <audio controls data-role="a"></audio>
                                    </div>
                                    <div class="ab-side ab-side-b">
                                      <span class="ab-label">B • After</span>
                                      <audio controls data-role="b"></audio>
                                    </div>
                                  </div>
                                `;
                            ...
                            let audioA = wrapper.querySelector('audio[data-role="a"]');
                            let audioB = wrapper.querySelector('audio[data-role="b"]');
                            if (!audioA) {
                                audioA = document.createElement('audio');
                                audioA.setAttribute('controls', '');
                                audioA.dataset.role = 'a';
                                wrapper.querySelector('.ab-side-a')?.appendChild(audioA);
                            }
                            if (!audioB) {
                                audioB = document.createElement('audio');
                                audioB.setAttribute('controls', '');
                                audioB.dataset.role = 'b';
                                wrapper.querySelector('.ab-side-b')?.appendChild(audioB);
                            }

                            // Apply critical attributes (CORS + preload)
                            ...
                            // Set/replace <source> safely
                            function setSource(audioEl, src) {
                                // If a <source> exists, update; else create
                                let source = audioEl.querySelector('source');
                                if (!source) {
                                    source = document.createElement('source');
                                    audioEl.appendChild(source);
                                }
                                source.src = src;
                                // Infer type by extension (basic)
                                const lower = src.toLowerCase();
                                let type = 'audio/mpeg';
                                if (lower.endsWith('.wav')) type = 'audio/wav';
                                else if (lower.endsWith('.ogg') || lower.endsWith('.oga')) type = 'audio/ogg';
                                ...
                                // Helpful logging + error hook
                                audioEl.addEventListener('error', () => {
                                    const err = audioEl.error;
                                    console.error('Mastering Lab: audio error', { src, err });
                                    errorHandler.displayError(`Audio failed to load: ${src}`);
                                }, { once: true });
                            }

                            setSource(audioA, aSrc);
                            setSource(audioB, bSrc);

                            // Optional: ensure only one plays at a time
                            function pauseOther(current) {
                                [audioA, audioB].forEach(aud => {
                                    if (aud !== current && !aud.paused) aud.pause();
                                });
                            }
                            audioA.addEventListener('play', () => pauseOther(audioA));
                            audioB.addEventListener('play', () => pauseOther(audioB));

                            // Optional: small UX—space toggles between A/B if focused
                            wrapper.addEventListener('keydown', (e) => {
                                if (e.code === 'Space') {
                                    e.preventDefault();
                                    if (document.activeElement === audioA || document.activeElement === audioB) {
                                        const other = (document.activeElement === audioA) ? audioB : audioA;
                                        if (!other.paused) other.pause();
                                    }
                                }
                            });

                            console.log(`Mastering Lab initialized for "${title}" with CORS+preload.`);
                        });
                    });
                })();
